...
A far-horizon backdrop plate: the Crown, a cluster of enormous spires on the skyline of the Meridian, seen across a huge distance through layered teal fog. A dense silhouette of towers and buttresses, taller in the middle, dissolving toward both edges of the frame; layered fog banks in front of and behind the cluster so the towers read as sitting at several different depths; and hot magenta light only at the very tips of the tallest spires - a handful of small points and their faint bloom, nothing else in the frame is magenta. THE FOG DOES THE WORK: this is the most distant thing in the game, so it is nearly all low-contrast value, closer to the fog than to the ink, and the magenta tips are the only saturated thing present. Painted, not flat: the silhouette edges are soft where fog eats them and crisper where a tower is nearer, the fog banks have gentle noise so they do not band, and there is a faint vertical striation in the tower mass suggesting structure without resolving it. WHAT TO FIX FROM THE PREVIOUS VERSION: flat teal triangles with hard edges on white, no fog and no magenta at all. This plate is drawn at 1024x256 and shown at about that size on screen.

Asset id `backdrop-crown-horizon`, category `backdrops`, canvas 1024x256 pixels, seed 161447.
...
Reference boards are attached: docs/concept-art/13-human-scale-monster-climb-grammar.png, docs/concept-art/12-kaiju-ship-level-anthology.png. **Match their surface quality, not
...
This asset occupies 60 x 15 world tiles. The shipped default camera (FAR, per docs/decisions.md entries 7 and 17) renders RIG — 1.7 tiles — at 3.7% of screen height, so on a 1280x800 screen this asset is about **1044.7 x 261.2 pixels**, next to a 30px-tall RIG. That is the only size that matters. Judge every decision there.

The canvas is 1024x256, about 1.0x the pixels this will occupy on screen, so any feature thinner than 2 canvas pixels vanishes when it is drawn.
...
Roles allowed for this asset: deep-teal, haze, ink, hot-magenta.
...
export function render(ctx, env) {
  // ctx: a CanvasRenderingContext2D, 1024x256, cleared to transparent.
