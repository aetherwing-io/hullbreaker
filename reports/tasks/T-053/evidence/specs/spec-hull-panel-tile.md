...
A seamless tile of the Meridian's exterior armour skin. THIS IS A RETRY: the previous recipe produced wavy organic seams that read as cracks, a pale rope-like highlight along the horizontal joint, and so many bolt heads that a tiled wall turned into busy noise. Fix all three. What it must be instead: MACHINED, QUIET METAL. The panel joints are dead straight lines - an armour plate is cut and bolted, not torn - one vertical joint and one horizontal joint, each a narrow dark recess with a soft occlusion falloff on both sides and a thin lit chamfer on the light side only. At most six bolt heads in the whole tile, in one short row beside the vertical joint, each about four pixels across, modelled with a lit crown and a dark contact shadow rather than drawn as a circle. Everything else is SURFACE: a broad value ramp across each plate face driven by low-frequency noise, fine directional grain running with the plate, a few long streaks of rust bleed and dirt running down from the joint, paint worn thin at the plate corners so cooler bare metal shows through. Light from the upper left. The tile is seen about thirty times at once on a wall, at 35 px per copy, so the eye must read a large calm plane with one seam crossing it - if a feature is countable at that size it is too strong. Four or five value steps between the joint floor and the worn highlight, and no single feature brighter than the worn metal.

Asset id `hull-panel-tile`, category `textures`, canvas 128x128 pixels, seed 371232.
