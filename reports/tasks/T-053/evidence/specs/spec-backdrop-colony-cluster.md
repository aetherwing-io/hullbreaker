...
A mid-distance backdrop plate: a human colony clinging to the Meridian's hull like a barnacle town - stacked shipping-crate habitats, gantries, mast aerials, a water tank, and a lattice of catwalks between them, all built out of salvaged rust-orange plate and grey hull metal. ROWS OF LIT WINDOWS ARE THE SCALE UNIT: small warm-white rectangles in regular rows, some lit and some dark, because that is what tells the player this thing is inhabited and how big it is. It sits at mid distance, so it keeps more contrast than the far horizon but still loses its darkest darks to the teal fog, more so at the edges of the cluster. Painted, not flat: grain and patchwork value variation across the plating (it is salvage - no two panels the same tone), occlusion under every overhang and catwalk, warm light spilling from the windows onto the surface just below them, rust streaks running down from the fixings. WHAT TO FIX FROM THE PREVIOUS VERSION: flat rectangles in three tones, windows as identical bright dots, no depth and no atmosphere. At true size this plate is 243x122 px on screen, so the whole cluster silhouette and the window rows are what read; individual panels are texture.

Asset id `backdrop-colony-cluster`, category `backdrops`, canvas 512x256 pixels, seed 597251.
...
This asset occupies 14 x 7 world tiles. The shipped default camera (FAR, per docs/decisions.md entries 7 and 17) renders RIG — 1.7 tiles — at 3.7% of screen height, so on a 1280x800 screen this asset is about **243.8 x 121.9 pixels**, next to a 30px-tall RIG. That is the only size that matters. Judge every decision there.

The canvas is 512x256, about 2.1x the pixels this will occupy on screen, so any feature thinner than 2 canvas pixels vanishes when it is drawn.
...
Roles allowed for this asset: haze, ink, hull, deep-teal, rust-orange, warm-white.
...
export function render(ctx, env) {
  // ctx: a CanvasRenderingContext2D, 512x256, cleared to transparent.
