...
A mid-distance backdrop plate: a gill cavity in the Meridian's flank - a deep rectangular recess in the armour with a rust-armoured lip, filled with a stack of angled louvre vanes running back into darkness, and a maintenance ladder plus a walkway rail crossing the lower part of the opening as the human-scale mark. THE DEPTH IS THE SUBJECT: the cavity gets darker and cooler the further in it goes, each vane throws a shadow onto the one behind it, the lip catches the key light from the upper left, and warm rust bleeds down the wall below the opening. Around the cavity, hull plating with grain and panel seams, dropping toward the teal fog at the edges of the plate. Painted, not flat: value gradient inside the cavity, soft occlusion in every vane gap and under the lip, streaking on the wall, dust haze pooling in the deepest part of the recess. WHAT TO FIX FROM THE PREVIOUS VERSION: flat vector louvres of one tone with hard edges and no sense of depth at all. At true size this plate is 418x418 px on screen.

Asset id `backdrop-gill-cavity`, category `backdrops`, canvas 512x512 pixels, seed 64351.
...
This asset occupies 24 x 24 world tiles. The shipped default camera (FAR, per docs/decisions.md entries 7 and 17) renders RIG — 1.7 tiles — at 3.7% of screen height, so on a 1280x800 screen this asset is about **417.9 x 417.9 pixels**, next to a 30px-tall RIG. That is the only size that matters. Judge every decision there.

The canvas is 512x512, about 1.2x the pixels this will occupy on screen, so any feature thinner than 2 canvas pixels vanishes when it is drawn.
...
Roles allowed for this asset: deep-teal, haze, hull, ink, rust-orange.
...
export function render(ctx, env) {
  // ctx: a CanvasRenderingContext2D, 512x512, cleared to transparent.
