...
An armoured vent cover set into the Meridian's hull, seen straight on: a heavy rust-armour frame with bolt heads at its corners, four horizontal louvre blades across the opening, and the lowest blade broken and bent open with acid-green spore light spilling out of the dark interior. Each blade is cooler bare metal with a lit top edge and a hard shadow cast onto the blade below; the cavity behind the blades is nearly black; the frame carries grain, paint worn back at the corners, and rust bleeding down from each bolt. WHAT TO FIX FROM THE PREVIOUS VERSION: flat rectangles, one flat green rectangle for the glow, no occlusion anywhere. At true size this plate is 26 px across, so the read is: dark rectangle, four bright horizontal lines, one green mouth at the bottom - the acid glow is the single accent and it must be small and the brightest thing present. This asset does not tile; it is one plate seen whole, and it should sit on the transparent background outside its own frame.

Asset id `vent-louver-plate`, category `textures`, canvas 128x128 pixels, seed 712521.
...
Reference boards are attached: docs/concept-art/01-exterior-gameplay.png, docs/concept-art/06-enemy-form-language.png. **Match their surface quality, not
...
This asset occupies 1.5 x 1.5 world tiles. The shipped default camera (FAR, per docs/decisions.md entries 7 and 17) renders RIG — 1.7 tiles — at 3.7% of screen height, so on a 1280x800 screen this asset is about **26.1 x 26.1 pixels**, next to a 30px-tall RIG. That is the only size that matters. Judge every decision there.

The canvas is 128x128, about 4.9x the pixels this will occupy on screen, so any feature thinner than 5 canvas pixels vanishes when it is drawn.
...
- **acid-green** (enemy glow, danger, hostile ecology)
  anchor `#a8c22a`, legal range: hue 100-150 in CIELCh
...
Roles allowed for this asset: rust-orange, ink, hull, haze, acid-green.
