...
A distant parallax backdrop plate: a vertebral spine column of the Meridian rising through the frame, seen through heavy teal fog. Stacked vertebra segments, each a banded drum with a dark recessed joint above and below it, rib spurs sweeping out to both sides and vanishing into the haze, and a maintenance gallery clinging to two of the segments with a row of small lit windows as the human-scale mark. ATMOSPHERIC PERSPECTIVE IS THE POINT: contrast and saturation fall off with distance and with height, the ribs furthest back are barely darker than the fog, and the fade is continuous and dithered so it does not band. Painted, not flat: grain across every drum, vertical streaking where fluids have run, soft occlusion in each joint and under each rib root. WHAT TO FIX FROM THE PREVIOUS VERSION: flat teal polygons on a white background with hard vector edges - it read as a diagram. This plate is drawn at 512x512 and shown on screen at about that size, so detail here is detail the player sees.

Asset id `backdrop-spine-coil`, category `backdrops`, canvas 512x512 pixels, seed 187307.
...
This asset occupies 30 x 30 world tiles. The shipped default camera (FAR, per docs/decisions.md entries 7 and 17) renders RIG — 1.7 tiles — at 3.7% of screen height, so on a 1280x800 screen this asset is about **522.4 x 522.4 pixels**, next to a 30px-tall RIG. That is the only size that matters. Judge every decision there.

The canvas is 512x512, about 1.0x the pixels this will occupy on screen, so any feature thinner than 2 canvas pixels vanishes when it is drawn.
...
Roles allowed for this asset: deep-teal, haze, ink, hull, warm-white.
