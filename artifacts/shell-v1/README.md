...
| File | What it shows |
| --- | --- |
| `title-wake.png` | direction 2 — "The Ship Wakes" (shipped default): the hull plate hinged out of the body, RIG running it at 3.8% of frame height, acid-lit interior below, magenta service lamps in the mass |
...
- **Board 13's scale rule holds on the title too.** RIG is 3.8% of frame
  height in all three directions (the concept-art range is 3–5%), asserted
  headlessly in `tools/pathcheck.mjs` — the composition data lives in
  `src/pure/shell.js` precisely so it can be checked rather than eyeballed.
