...
/* The Node-safe half of the runtime sprite path (T-049): which PNG stands
   ...
   canvas — a wasp's art covers 94% x 88% of a 32x32 PNG. Sizing the quad to
   the canvas therefore draws the BODY smaller than the primitive it
...
/* The tripod is the one role whose art is squarer than the mesh it
   ...
   Every other role's quad matches the primitive box exactly.            */
export const MORTAR_STANCE = 1.5;

/* The box the PRIMITIVE mesh draws, in tiles, and where its center sits
   relative to the sim row — the target the sprite's ink is fitted to.
...
/* The quad: how big the PlaneGeometry is in tiles, and how far its center
   sits from the sim row so that the art's INK lands exactly on the
   primitive box. Pure arithmetic over the two tables above — no clamping,
   no taste, and no sim field anywhere in it.
