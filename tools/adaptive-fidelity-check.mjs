...
const tune = {
  ...
  avgBudgetMs: 19.5, slowFrameMs: 24, slowShare: 0.20, maxLevel: 3,
};
...
assert.equal(feed(30, 4).level, 1);
...
assert.match(runtime, /QUERY\.get\('adaptive'\) !== '0'/,
  'adaptive quality has an explicit measurement escape hatch');
...
assert.match(main, /resetGame\(\);\s*warmScenePrograms\(\);/,
  'program compilation happens after initial run construction and before frame one');
