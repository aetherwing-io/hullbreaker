...
Nine scripts are committed under `scripts/`, with their reports committed
under `reports/demo/` (screenshots/videos are gitignored; the JSON + summary
are the actual demo artifact) — except the two six-face policy runs, whose
evidence lives with the finding that measured them, under
`reports/t019/`. All nine run in **`testapi` fidelity**. The
