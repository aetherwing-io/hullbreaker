...
- URL: `http://127.0.0.1:8756/index.html?enemies=0&testapi=1`
- Started: 2026-08-02T02:42:14.551Z
- Wall time: 16.3s
- Fidelity: **testapi**
- Sampling: requested every 75ms, achieved avg 76.1ms / max 163ms (212 samples)
...
- Kills: 3, hits survived: 0
- Stock lives (source: telemetry): 3 → 3, **0 spent**

## Pacing / fairness metrics
- Idle time (A.5 `stallMs`): 0.1s of 16.1s PLAYING time (fraction 0.005)
- Airborne time (`airMs`): 13.7s
- Closest approach to crush edge (`minEdgeMargin`): 32.8 tiles
- Vertical range: y 0.58–9.01 (span 8.43)
...
- `?enemies=0`: **NO-OP on this run** — ?enemies=0 sets SLICE_ENEMIES_ENABLED (src/mode.js), which is read in exactly one place: src/sim/spawner.js, where a FIXTURE spawns its authored list. It is SLICE-ONLY — the default six-face run's ambient spawner never consults it (SPRINT I-026). On this run it did NOT hold: up to 3 live hostile rows (carrier, hound, wasp) were present on 212 of 212 sampled ticks, and the served build is the default-run. Any per-gap, pacing or damage number taken from this run inherits hostile contact it was authored to exclude.
- Input density (A.5: deliberately NOT a score input): 4.78 events/sec (78 total: 39 down / 39 up)
